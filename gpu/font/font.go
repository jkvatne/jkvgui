...
func (f *Font) DrawText(Gd gpu.GlData, x, y float32, color f32.Color, maxW float32, dir gpu.Direction, str string) {
	runes := []rune(str)
	if len(runes) == 0 {
		return
	}
	...
	y *= Gd.ScaleY
	maxW *= Gd.ScaleX
	size := Gd.ScaleX * DefaultDpi / f.dpi
	gpu.SetupTexture(color, Gd.FontVao, Gd.FontVbo, Gd.FontProgram)
	...
	for ch := low; ch <= high; ch++ {
		...
		if *DebugFonts {
			if ch == 'E' {
				...
				slog.Info("Letter E", "w", char.width, "h", char.height, "dpi", f.dpi, "default dpi", DefaultDpi, "f.size", f.Size)
...
func LoadFontBytes(no int, name string, data []byte, size int, weight float32, dpi float32) {
	...
	_ = f.GenerateGlyphs(Ellipsis, Ellipsis)
	Fonts[no] = f
}
